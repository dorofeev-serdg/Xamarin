...
namespace Droid
{
	public class SlidingTabStrip : LinearLayout
	{
		...
		public SlidingTabStrip(Context context, IAttributeSet attrs) : base(context, attrs)
		{
			...
			mDefaultBottmoBorderColor = SetColorAlpha(themeForeGround, DEFAULT_BOTTOM_BORDER_COLOR_ALPHA);

			mDefaultTabColorizer = new SimpleTabColorizer ();
			mDefaultTabColorizer.IndicatoColors = INDICATOR_COLORS;
			...
			mDividerPaint.StrokeWidth = (int)(DEFAULT_DIVIDER_THICHNESS_DIPS * density);
		}
		...
		public int [] SelectedIndicatorColors
		{
			...
			{ 
				...
				mDefaultTabColorizer.IndicatoColors = value;
				this.Invalidate ();
			}
		}
		...
		protected override void OnDraw(Canvas canvas)
		{
			...
			if (tabCount > 0) {
				View selecteTitle = GetChildAt (mSelectedPosition);
				int left = selecteTitle.Left;
				int right = selecteTitle.Right;
